...
- **CQRobot DMX shield + lid posts on every joint** ([#27](https://github.com/SlyWombat/case-maker/issues/27)): verified the CQRobot DMX shield JSON against the CTC-DRA-10-R2 datasheet — corrected PCB to 65 × 54 mm (was 68.6 × 53.4), removed the non-existent `screw-terminal` component, repositioned the two NEUTRIK XLR-3 connectors (24 mm round) to match the real product, dropped the synthetic "flipped 180°" mounting position (the shield's headers only fit one way). Refactored `buildLid` so all four joint variants (`flat-lid`, `snap-fit`, `sliding`, `screw-down`) carry downward posts at the boss positions; only `screw-down` adds screw-clearance holes through them. New `'none'` member of `InsertType` produces solid posts and solid floor bosses (no pilot hole) — the system auto-applies this when the joint isn't `screw-down`, so switching joint preserves board retention without leaving useless screw holes. New `tests/unit/lidPostsAcrossJoints.spec.ts` (5 cases) asserts the contract.

### Added

- **External library references** in `docs/board-library-audit.md`: KiCad symbol/footprint libraries, the official Pi HAT+ specification (RP-008281-DS-1), and Arduino EAGLE/KiCad sources. These are the authoritative cross-reference set for the per-board manual audit.
